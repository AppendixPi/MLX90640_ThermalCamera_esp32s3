...
static const char *TAG_i2c = "MLX90640_i2c";
...
int MLX90640_I2CRead(uint8_t slaveAddr, uint16_t startAddress, uint16_t nMemAddressRead, uint16_t *data)
{
    ...
    if(ret != ESP_OK)
    {
        ESP_LOGI(TAG_i2c, "I2C READ ERROR %i", ret);
    }
    return ret;
}
...
int MLX90640_I2CGeneralReset(void)
{
    ...
    i2c_master_start(cmd); // Add a start condition
    i2c_master_write_byte(cmd, 0x00 << 1 | I2C_MASTER_WRITE, true); // Address + Write bit
    i2c_master_write_byte(cmd, reset_command, true); // Write the reset command
    i2c_master_stop(cmd); // Add a stop condition
    esp_err_t ret = i2c_master_cmd_begin(I2C_NUM_0, cmd, pdMS_TO_TICKS(1000)); // Execute the command link
    i2c_cmd_link_delete(cmd); // Delete the command link

    if (ret == ESP_OK) {
        ESP_LOGI(TAG_i2c, "I2C general reset successful");
        return 0;
    } else {
    	ESP_LOGI(TAG_i2c, "I2C general reset failed");
        return -1;
    }
}
